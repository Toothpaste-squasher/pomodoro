...
export const createSesh = async (req, res) => {
  const sql = `INSERT INTO sessions (user_id, session_type, end_time, duration_s, notes) VALUES (?, ?, ?, ?, ?)`
  const { session_type, end_time, duration_s, notes } = req.body.newSesh
  const user_id = req.user.id
  await pool.execute(sql, [user_id, session_type, end_time, duration_s, notes])
